...
async function main() {
  ...
  const orderItems = await Promise.all([
    // Order 1 (Bàn 1) - completed
    prisma.orderItem.create({
      data: {
        orderId: orders[0].id,
        menuItemId: allMenuItems[0].id, // Khoai tây chiên
        quantity: 1,
        note: 'Nhiều dầu'
      }
    }),
    prisma.orderItem.create({
      data: {
        orderId: orders[0].id,
        menuItemId: allMenuItems[9].id, // Phở bò
        quantity: 2,
        note: ''
      }
    }),
    prisma.orderItem.create({
      data: {
        orderId: orders[0].id,
        menuItemId: allMenuItems[33].id, // Bạc xỉu
        quantity: 2,
        note: ''
      }
    }),

    // Order 2 (Bàn 2) - cooking
    prisma.orderItem.create({
      data: {
        orderId: orders[1].id,
        menuItemId: allMenuItems[19].id, // Gà hấm
        quantity: 1,
        note: 'Nóng'
      }
    }),
    prisma.orderItem.create({
      data: {
        orderId: orders[1].id,
        menuItemId: allMenuItems[2].id, // Bánh cuốn
        quantity: 1,
        note: ''
      }
    }),
    prisma.orderItem.create({
      data: {
        orderId: orders[1].id,
        menuItemId: allMenuItems[44].id, // Bia HN
        quantity: 2,
        note: 'Đá kèm'
      }
    }),
    ...
    prisma.orderItem.create({
      data: {
        orderId: orders[2].id,
        menuItemId: allMenuItems[11].id, // Cơm tấm
        quantity: 1,
        note: 'Thêm trứng ốp'
      }
    }),
    prisma.orderItem.create({
      data: {
        orderId: orders[2].id,
        menuItemId: allMenuItems[35].id, // Nước cam
        quantity: 2,
        note: ''
      }
    }),

    // Order 4 (Bàn VIP) - pending
    prisma.orderItem.create({
      data: {
        orderId: orders[3].id,
        menuItemId: allMenuItems[23].id, // Vịt quay
        quantity: 1,
        note: 'Ăn kèm chóp'
      }
    }),
    prisma.orderItem.create({
      data: {
        orderId: orders[3].id,
        menuItemId: allMenuItems[1].id, // Chả giò
        quantity: 2,
        note: 'Thêm nước chấm'
      }
    }),
  ]);
  console.log(`✓ Đã tạo ${orderItems.length} item đơn hàng.`);
